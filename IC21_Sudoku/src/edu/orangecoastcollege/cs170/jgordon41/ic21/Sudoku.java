			...
			try {
				...
				case "S":
					...
					if (workingPuzzle[row][col] != 0)
						throw new InvalidMoveException("Please enter a space that is not taken!!");
					else
						workingPuzzle[row][col] = valueChange;
					break;
				case "R":
				...
				case "Q":
					...
					break;
				default:
					System.out.println("Sorry i dont recognize that.");
					break;
				}
			} catch (InvalidMoveException e) {
				System.out.println(e.getMessage());
			}catch(ArrayIndexOutOfBoundsException e){
				System.out.println("Too Many rows or Colums entered, please enter again");
				error = true;
			}
		} while (!input.equalsIgnoreCase("Q") && checkGame(workingPuzzle) || error);
		
		if(workingPuzzle[row][col] == solvedPuzzle[row][col] && solved == false){
			System.out.println("You ran out of moves, Thanks for playing");
		}
		else if(solved == true)
 ...
 */
	public static boolean checkGame(int[][] workingPuzzle){
		...
		for(int row = 0; row < workingPuzzle.length; row++){
			check = true;
			for (int col = 0; col < workingPuzzle[row].length;col++){
				check = true;
				if(workingPuzzle[row][col] != 0){
					check = false;
					break;
				}
				if(workingPuzzle[row][col] == solvedPuzzle[row][col]){
					solved = true;
				}
			}
			if(!check)
...
>>>>>>> branch 'Home' of https://github.com/jdog1218/Java1.git
